...
class DefaultFillerMixin(MemoryMixin):
    ...
    def _default_value_improve(self, addr, size, name=None, inspect=True, events=True, key=None, fill_missing: bool=True,
                       **kwargs):
        if self.state.project and self.state.project.concrete_target \
            and size == 1 and addr < options.SYMBOLIC_READ_MEMORY_MAX_ADDR and addr > options.SYMBOLIC_READ_MEMORY_MIN_ADDR:
    ...
    def _default_pointer_value(self, addr, size, name=None, inspect=True, events=True, key=None, fill_missing: bool=True,
                       **kwargs): # 支持指针类型自动符号化处理
        if self.state.project and self.state.project.concrete_target and addr <= 0xff000000 and addr >= 0x800000:
            mem = self.state.project.concrete_target.read_memory(addr, size)
            endness = kwargs["endness"]
            bvv = self.state.solver.BVV(mem)
            return bvv if endness == 'Iend_BE' else bvv.reversed
